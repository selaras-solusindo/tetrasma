...
$level2_view = NULL; // Initialize page object first

class clevel2_view extends clevel2 {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["level2_id"] <> "") {
				...
				$sReturnUrl = "level2list.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "level2list.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "level2list.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_level2\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_level2',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.flevel2view,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($level2_view)) $level2_view = new clevel2_view();

// Page init
$level2_view->Page_Init();

// Page main
$level2_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$level2_view->Page_Render();
?>
...
var CurrentForm = flevel2view = new ew_Form("flevel2view", "view");

// Form_CustomValidate event
flevel2view.Form_CustomValidate = 
...
flevel2view.ValidateRequired = true;
<?php } else { ?>
flevel2view.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
flevel2view.Lists["x_level1_id"] = {"LinkField":"x_level1_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_level1_no","x_level1_nama","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"level1"};
...
<?php if ($level2->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$level2_view->IsModal) { ?>
<?php if ($level2->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $level2_view->ExportOptions->Render("body") ?>
<?php
	foreach ($level2_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$level2_view->IsModal) { ?>
<?php if ($level2->Export == "") { ?>
...
<?php } ?>
<?php $level2_view->ShowPageHeader(); ?>
<?php
$level2_view->ShowMessage();
?>
<form name="flevel2view" id="flevel2view" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($level2_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $level2_view->Token ?>">
<?php } ?>
...
<?php if ($level2->level1_id->Visible) { // level1_id ?>
	<tr id="r_level1_id">
		<td><span id="elh_level2_level1_id"><?php echo $level2->level1_id->FldCaption() ?></span></td>
...
<?php echo $level2->level1_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($level2->level2_no->Visible) { // level2_no ?>
	<tr id="r_level2_no">
		<td><span id="elh_level2_level2_no"><?php echo $level2->level2_no->FldCaption() ?></span></td>
...
<?php echo $level2->level2_no->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($level2->level2_nama->Visible) { // level2_nama ?>
	<tr id="r_level2_nama">
		<td><span id="elh_level2_level2_nama"><?php echo $level2->level2_nama->FldCaption() ?></span></td>
...
<?php echo $level2->level2_nama->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
