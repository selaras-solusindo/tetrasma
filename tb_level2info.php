...
//
// Table class for tb_level2
//
class ctb_level2 extends cTable {
	var $level2_id;
	var $level1_id;
	var $level2_no;
	var $level2_nama;
	...
	function __construct() {
		...
		$this->TableVar = 'tb_level2';
		$this->TableName = 'tb_level2';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`tb_level2`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// level2_id
		$this->level2_id = new cField('tb_level2', 'tb_level2', 'x_level2_id', 'level2_id', '`level2_id`', '`level2_id`', 3, -1, FALSE, '`level2_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// level1_id
		$this->level1_id = new cField('tb_level2', 'tb_level2', 'x_level1_id', 'level1_id', '`level1_id`', '`level1_id`', 3, -1, FALSE, '`EV__level1_id`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'TEXT');
		...
		// level2_no
		$this->level2_no = new cField('tb_level2', 'tb_level2', 'x_level2_no', 'level2_no', '`level2_no`', '`level2_no`', 200, -1, FALSE, '`level2_no`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->level2_no->Sortable = TRUE; // Allow sort
		$this->fields['level2_no'] = &$this->level2_no;

		// level2_nama
		$this->level2_nama = new cField('tb_level2', 'tb_level2', 'x_level2_nama', 'level2_nama', '`level2_nama`', '`level2_nama`', 200, -1, FALSE, '`level2_nama`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`tb_level2`";
	}
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT CONCAT(`level1_no`,'" . ew_ValueSeparator(1, $this->level1_id) . "',`level1_nama`) FROM `tb_level1` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`level1_id` = `tb_level2`.`level1_id` LIMIT 1) AS `EV__level1_id` FROM `tb_level2`" .
			") `EW_TMP_TABLE`";
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("tb_level2view.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("tb_level2view.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "tb_level2add.php?" . $this->UrlParm($parm);
		else
			$url = "tb_level2add.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("tb_level2edit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("tb_level2add.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("tb_level2delete.php", $this->UrlParm());
	}
