...
$tb_akun_lvl3_add = NULL; // Initialize page object first

class ctb_akun_lvl3_add extends ctb_akun_lvl3 {
	...
	// Page object name
	var $PageObjName = 'tb_akun_lvl3_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["akun_lvl3_id"] != "") {
				...
				$this->setKey("akun_lvl3_id", $this->akun_lvl3_id->CurrentValue); // Set up key
			} else {
				$this->setKey("akun_lvl3_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "tb_akun_lvl3view.php")
	...
	function LoadDefaultValues() {
		...
		$this->akun_lvl3_nama->CurrentValue = NULL;
		$this->akun_lvl3_nama->OldValue = $this->akun_lvl3_nama->CurrentValue;
	}
	...
	function LoadOldRecord() {
		...
		$bValidKey = TRUE;
		if (strval($this->getKey("akun_lvl3_id")) <> "")
			$this->akun_lvl3_id->CurrentValue = $this->getKey("akun_lvl3_id"); // akun_lvl3_id
		else
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		// akun_lvl3_nama
		$this->akun_lvl3_nama->SetDbValueDef($rsnew, $this->akun_lvl3_nama->CurrentValue, "", FALSE);
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->akun_lvl3_id->setDbValue($conn->Insert_ID());
				$rsnew['akun_lvl3_id'] = $this->akun_lvl3_id->DbValue;
			}
		} else {
...
// Create page object
if (!isset($tb_akun_lvl3_add)) $tb_akun_lvl3_add = new ctb_akun_lvl3_add();

// Page init
$tb_akun_lvl3_add->Page_Init();

// Page main
$tb_akun_lvl3_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tb_akun_lvl3_add->Page_Render();
?>
...
var CurrentForm = ftb_akun_lvl3add = new ew_Form("ftb_akun_lvl3add", "add");

// Validate form
ftb_akun_lvl3add.Validate = function() {
...
ftb_akun_lvl3add.Form_CustomValidate = 
...
ftb_akun_lvl3add.ValidateRequired = true;
<?php } else { ?>
ftb_akun_lvl3add.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftb_akun_lvl3add.Lists["x_akun_lvl1_id"] = {"LinkField":"x_akun_lvl1_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_akun_lvl1_nama","","",""],"ParentFields":[],"ChildFields":["x_akun_lvl2_id"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tb_akun_lvl1"};
ftb_akun_lvl3add.Lists["x_akun_lvl2_id"] = {"LinkField":"x_akun_lvl2_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_akun_lvl2_nama","","",""],"ParentFields":["x_akun_lvl1_id"],"ChildFields":[],"FilterFields":["x_akun_lvl1_id"],"Options":[],"Template":"","LinkTable":"tb_akun_lvl2"};
...
<?php if (!$tb_akun_lvl3_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $tb_akun_lvl3_add->ShowPageHeader(); ?>
<?php
$tb_akun_lvl3_add->ShowMessage();
?>
<form name="ftb_akun_lvl3add" id="ftb_akun_lvl3add" class="<?php echo $tb_akun_lvl3_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tb_akun_lvl3_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tb_akun_lvl3_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tb_akun_lvl3_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
ftb_akun_lvl3add.Init();
</script>
<?php
$tb_akun_lvl3_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tb_akun_lvl3_add->Page_Terminate();
?>
