...
$tb_detail_add = NULL; // Initialize page object first

class ctb_detail_add extends ctb_detail {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "tb_detailview.php")
	...
	function AddRow($rsold = NULL) {
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$rsnew['detail_id'] = $this->detail_id->DbValue;
			}
		} else {
...
// Create page object
if (!isset($tb_detail_add)) $tb_detail_add = new ctb_detail_add();

// Page init
$tb_detail_add->Page_Init();

// Page main
$tb_detail_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tb_detail_add->Page_Render();
?>
...
var CurrentForm = ftb_detailadd = new ew_Form("ftb_detailadd", "add");

// Validate form
ftb_detailadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($tb_detail->akun_id->FldErrMsg()) ?>");
...
ftb_detailadd.Form_CustomValidate = 
...
ftb_detailadd.ValidateRequired = true;
<?php } else { ?>
ftb_detailadd.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $tb_detail_add->ShowPageHeader(); ?>
<?php
$tb_detail_add->ShowMessage();
?>
<form name="ftb_detailadd" id="ftb_detailadd" class="<?php echo $tb_detail_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tb_detail_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tb_detail_add->Token ?>">
<?php } ?>
...
<?php if ($tb_detail->akun_id->Visible) { // akun_id ?>
	<div id="r_akun_id" class="form-group">
		<label id="elh_tb_detail_akun_id" for="x_akun_id" class="col-sm-2 control-label ewLabel"><?php echo $tb_detail->akun_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $tb_detail->akun_id->CellAttributes() ?>>
<span id="el_tb_detail_akun_id">
<input type="text" data-table="tb_detail" data-field="x_akun_id" name="x_akun_id" id="x_akun_id" size="30" placeholder="<?php echo ew_HtmlEncode($tb_detail->akun_id->getPlaceHolder()) ?>" value="<?php echo $tb_detail->akun_id->EditValue ?>"<?php echo $tb_detail->akun_id->EditAttributes() ?>>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tb_detail_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
