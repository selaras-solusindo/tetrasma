...
$level4_delete = NULL; // Initialize page object first

class clevel4_delete extends clevel4 {
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("level4list.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in level4 class, level4info.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("level4list.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($level4_delete)) $level4_delete = new clevel4_delete();

// Page init
$level4_delete->Page_Init();

// Page main
$level4_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$level4_delete->Page_Render();
?>
...
var CurrentForm = flevel4delete = new ew_Form("flevel4delete", "delete");

// Form_CustomValidate event
flevel4delete.Form_CustomValidate = 
...
flevel4delete.ValidateRequired = true;
<?php } else { ?>
flevel4delete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
flevel4delete.Lists["x_level1_id"] = {"LinkField":"x_level1_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_level1_nama","","",""],"ParentFields":[],"ChildFields":["x_level2_id"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"level1"};
flevel4delete.Lists["x_level2_id"] = {"LinkField":"x_level2_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_level2_nama","","",""],"ParentFields":[],"ChildFields":["x_level3_id"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"level2"};
flevel4delete.Lists["x_level3_id"] = {"LinkField":"x_level3_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_level3_nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"level3"};
...
<?php $level4_delete->ShowPageHeader(); ?>
<?php
$level4_delete->ShowMessage();
?>
<form name="flevel4delete" id="flevel4delete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($level4_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $level4_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="level4">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($level4_delete->RecKeys as $key) { ?>
...
<?php if ($level4->level1_id->Visible) { // level1_id ?>
		<th><span id="elh_level4_level1_id" class="level4_level1_id"><?php echo $level4->level1_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($level4->level2_id->Visible) { // level2_id ?>
		<th><span id="elh_level4_level2_id" class="level4_level2_id"><?php echo $level4->level2_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($level4->level3_id->Visible) { // level3_id ?>
		<th><span id="elh_level4_level3_id" class="level4_level3_id"><?php echo $level4->level3_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($level4->level4_no->Visible) { // level4_no ?>
		<th><span id="elh_level4_level4_no" class="level4_level4_no"><?php echo $level4->level4_no->FldCaption() ?></span></th>
<?php } ?>
<?php if ($level4->level4_nama->Visible) { // level4_nama ?>
		<th><span id="elh_level4_level4_nama" class="level4_level4_nama"><?php echo $level4->level4_nama->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$level4_delete->RecCnt = 0;
$i = 0;
while (!$level4_delete->Recordset->EOF) {
	$level4_delete->RecCnt++;
	$level4_delete->RowCnt++;

	// Set row properties
	$level4->ResetAttrs();
	$level4->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$level4_delete->LoadRowValues($level4_delete->Recordset);

	// Render row
	$level4_delete->RenderRow();
?>
	<tr<?php echo $level4->RowAttributes() ?>>
<?php if ($level4->level1_id->Visible) { // level1_id ?>
		<td<?php echo $level4->level1_id->CellAttributes() ?>>
<span id="el<?php echo $level4_delete->RowCnt ?>_level4_level1_id" class="level4_level1_id">
...
<?php } ?>
<?php if ($level4->level2_id->Visible) { // level2_id ?>
		<td<?php echo $level4->level2_id->CellAttributes() ?>>
<span id="el<?php echo $level4_delete->RowCnt ?>_level4_level2_id" class="level4_level2_id">
...
<?php } ?>
<?php if ($level4->level3_id->Visible) { // level3_id ?>
		<td<?php echo $level4->level3_id->CellAttributes() ?>>
<span id="el<?php echo $level4_delete->RowCnt ?>_level4_level3_id" class="level4_level3_id">
...
<?php } ?>
<?php if ($level4->level4_no->Visible) { // level4_no ?>
		<td<?php echo $level4->level4_no->CellAttributes() ?>>
<span id="el<?php echo $level4_delete->RowCnt ?>_level4_level4_no" class="level4_level4_no">
...
<?php } ?>
<?php if ($level4->level4_nama->Visible) { // level4_nama ?>
		<td<?php echo $level4->level4_nama->CellAttributes() ?>>
<span id="el<?php echo $level4_delete->RowCnt ?>_level4_level4_nama" class="level4_level4_nama">
...
<?php
	$level4_delete->Recordset->MoveNext();
}
$level4_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $level4_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
flevel4delete.Init();
