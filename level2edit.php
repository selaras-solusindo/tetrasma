...
<?php
$wrkonchange = trim(" " . @$level2->level1_id->EditAttrs["onchange"]);
if ($wrkonchange <> "") $wrkonchange = " onchange=\"" . ew_JsEncode2($wrkonchange) . "\"";
$level2->level1_id->EditAttrs["onchange"] = "";
?>
<span id="as_x_level1_id" style="white-space: nowrap; z-index: 8980">
	<input type="text" name="sv_x_level1_id" id="sv_x_level1_id" value="<?php echo $level2->level1_id->EditValue ?>" size="30" placeholder="<?php echo ew_HtmlEncode($level2->level1_id->getPlaceHolder()) ?>" data-placeholder="<?php echo ew_HtmlEncode($level2->level1_id->getPlaceHolder()) ?>"<?php echo $level2->level1_id->EditAttributes() ?>>
</span>
<input type="hidden" data-table="level2" data-field="x_level1_id" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $level2->level1_id->DisplayValueSeparatorAttribute() ?>" name="x_level1_id" id="x_level1_id" value="<?php echo ew_HtmlEncode($level2->level1_id->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<input type="hidden" name="q_x_level1_id" id="q_x_level1_id" value="<?php echo $level2->level1_id->LookupFilterQuery(true) ?>">
<script type="text/javascript">
flevel2edit.CreateAutoSuggest({"id":"x_level1_id","forceSelect":true});
</script>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($level2->level1_id->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x_level1_id',m:0,n:10,srch:false});" class="ewLookupBtn btn btn-default btn-sm"><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" name="s_x_level1_id" id="s_x_level1_id" value="<?php echo $level2->level1_id->LookupFilterQuery(false) ?>">
</span>
