...
$tb_level4_add = NULL; // Initialize page object first

class ctb_level4_add extends ctb_level4 {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "tb_level4view.php")
...
// Create page object
if (!isset($tb_level4_add)) $tb_level4_add = new ctb_level4_add();

// Page init
$tb_level4_add->Page_Init();

// Page main
$tb_level4_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tb_level4_add->Page_Render();
?>
...
var CurrentForm = ftb_level4add = new ew_Form("ftb_level4add", "add");

// Validate form
ftb_level4add.Validate = function() {
...
ftb_level4add.Form_CustomValidate = 
...
ftb_level4add.ValidateRequired = true;
<?php } else { ?>
ftb_level4add.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $tb_level4_add->ShowPageHeader(); ?>
<?php
$tb_level4_add->ShowMessage();
?>
<form name="ftb_level4add" id="ftb_level4add" class="<?php echo $tb_level4_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tb_level4_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tb_level4_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tb_level4_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
